...
from src.redeemer.errors import NotAuthorizedException, RefreshAuthFailedException
from src.redeemer.redeemer import CodeState, Redeemer
...
class Wolt(Redeemer):
    def __init__(self, db: Database, account_name: str):
        ...
        try:
            ...
            print(
                "account was not found in database, please run create_wolt_account.py script to create new account."
            )
            exit(1)
    ...
    def create_new_account(db: Database, account_name: str, refresh_token: str):
        ...
        with db.transaction:
            ...
            db.wolt_token.insert(
                account_id, token.refresh_token, token.access_token, token.expires_in
            )
    ...
    def get_new_token(self):
        self.actual_token = Wolt.make_request_to_new_token(
    ...
    def redeem_code(self, code: str) -> CodeState:
        ...
        if response.status_code == 401:
            raise NotAuthorizedException(
                "Wolt is not authorized, use get_new_token function."
            )
    ...
    def is_token_expired(self) -> bool:
        return (
            self.actual_token.created
            + timedelta(seconds=self.actual_token.expires_in - 10)
            <= datetime.utcnow()
        )  # - 10 is buffer for delay

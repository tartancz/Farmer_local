...
class YoutubeApi:
    def __init__(
        self, api_key: str, channel_id: str, channel_name: str, max_checks_per_day=7000
    ):
        self.API_KEY = api_key
        self.channel_id = channel_id
        self.channel_name = channel_name
        self._reset_unit_datetime: datetime = (
            YoutubeApi._get_next_reset_units_datetime()
        )
    ...
    def get_video_count(self) -> int:
        ...
        logger.debug(
            f"video count is {video_count} and used points is {self._used_points}"
        )
        return video_count
    ...
    def get_detailed_video(self, video_id: str) -> DetailedVideoFromApi:
        logger.debug(
            f"getting detailed video {video_id} and actual used points are {self._used_points}"
        )
        resp = (
            self.ytb.videos().list(part="snippet,contentDetails", id=video_id).execute()
        )
        if len(resp["items"]) < 1:
            raise VideoDoNotExistException(
                f"VideoFromApi with video_id {video_id} do not exist"
            )
        item = resp["items"][0]
    ...
    def get_latest_videos_from_api(self, count=1) -> list[VideoFromApi]:
        logger.debug(
            f"getting latest videos from api {count} and actual used points are {self._used_points}"
        )
        resp = (
            self.ytb.search()
            .list(
    ...
    def get_latest_videos_from_scrapping(self, count: int) -> list[str]:
        ...
        for script in bs.find_all("script"):
            ...
            x = json_data["contents"]["twoColumnBrowseResultsRenderer"]["tabs"][1][
                "tabRenderer"
            ]["content"]["richGridRenderer"]
            videos_id = []
            for vid in range(count):
                videos_id.append(
                    x["contents"][vid]["richItemRenderer"]["content"]["videoRenderer"][
                        "videoId"
                    ]
                )
            return videos_id
